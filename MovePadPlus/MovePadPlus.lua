...
local function SettoolTips()
	NS.c2mButton:SetScript("OnEnter", function(self)
		if MovePadPlus.HideTooltips then
			return
		end
...
function NS:InitialisEverything(c2mBtn)
	if MovePadFrame.SetPropagateMouseClicks then
		MovePadFrame:SetPropagateMouseClicks(true)
	end
...
--	c2mBtn:SetPoint("TOPRIGHT", MovePadBackward, "BOTTOMLEFT")
	...
	if NS.holdRotateButton.SetPropagateMouseClicks then
		NS.holdRotateButton:SetPropagateMouseClicks(true)
	end
	NS:SetShown(MovePadPlus.Buttons, "HideHoldRotate")
