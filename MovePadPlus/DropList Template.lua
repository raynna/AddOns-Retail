...
local function CreateTooltipBackdrop(name, parent, ftype)
	...
	local f = CreateFrame(ftype, name, parent, BackdropTemplateMixin and "BackdropTemplate")
	f:SetFrameStrata("BACKGROUND") 
	f:SetBackdrop(backdrop)
	return f
end

local function CreateText(frame, name, font, text, layer, frompoint, topoint, xoffset, yoffset, justifyH, justifyY, size, red, green, blue, alpha)
...
local function CreateDropMenu(name, parent, label, table, maxdrops, data, index, width, height, enabled, labelright, override, strata)
	...
	f.label = CreateText(f, name .. "Label", "Fonts/ARIALN.ttf", label, "ARTWORK", left, right, xoffset, 0, "RIGHT", "MIDDLE", 12, 1, 1, 1)
	f.setting = CreateText(f, name .. "Setting", "Fonts/ARIALN.ttf", "", "ARTWORK", "LEFT", "LEFT", 3, 0, "LEFT", "MIDDLE", 12, 1, 1, 1)
...
local function CreateDropButton(name, parent, table, data, index)
	...
	f.text = CreateText(f, name .. "_Text", "Fonts/ARIALN.ttf", "", "ARTWORK", "LEFT", "LEFT", 0, 0, "LEFT", "MIDDLE", 12)
