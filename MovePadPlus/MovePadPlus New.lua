...
NS.Texts = {
	...
	MLCursorShow = "MouseLook Cursor",
	MLCursorAnimate = "Animate",
	MLCursorColor = "Color",
}
...
local FrameButtonMixin = {}
function FrameButtonMixin:SetNormalTexture(texture, blend)
	self.NormalTexture:SetTexture(texture)
	if blend then
		self.NormalTexture:SetBlendMode(blend)
	end
end

function FrameButtonMixin:SetChecked(checked)
	self.CheckedTexture:SetShown(checked)
end

function FrameButtonMixin:SetCheckedTexture(texture, blend)
	self.CheckedTexture:SetTexture(texture)
	if blend then
		self.CheckedTexture:SetBlendMode(blend)
	end
end

local function FrameButtonSetup(self)
...
function NS:InitialisEverything(c2mBtn)
	...
	if not MovePadPlus.Cursor then 
		MovePadPlus.Cursor = {
			show = nil,
			color = { r=1, g=1, b=0 },
			minscale = 0.6,
...
--	NS.holdRotateButton = CreateFrame("CheckButton", "MovePadPlusHoldRotate", MovePadFrame, "UIPanelSquareButton")
	NS.holdRotateButton = CreateFrame("Frame", "MovePadPlusHoldRotate", MovePadFrame, "UIPanelSquareButton")
	FrameButtonSetup(NS.holdRotateButton)
	Mixin(NS.holdRotateButton, FrameButtonMixin)
	NS:SetShown(MovePadPlus.Buttons, "HideHoldRotate")
...
--[[
	...
	function NS.holdRotateButton:OnMouseUp(self, button, up)
			if not self.RotateOffOnUp then return end
			self:SetNormalTexture(self.NTexture)
			self.RotateOffOnUp = nil
			MouselookStop()
